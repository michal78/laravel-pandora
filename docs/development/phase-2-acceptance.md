...
> Status: in progress. Nothing below is ticked on the strength of code existing; each criterion is
> ticked only when a named automated test asserts it and that test passes.
...
| 11 | Argument modification is applied, recorded as a diff on the trace, and audited | `Tools/ArgumentModificationTest` |
...
| 19 | An expired approval moves the run to `failed` with a specific reason, not a generic error | `Approvals/ApprovalExpiryTest` |
...
| 23 | Exactly one `ContinueAgentRun` is dispatched after N parallel tool calls complete | `Queue/ToolFanInTest` |
| 24 | Exceeding `max_tool_calls` terminates the run as `timed_out` | `Tools/ToolBudgetTest` |
...
| 29 | Raw tool arguments and results are hidden from a user without `pandora.tools.io.view` | `Security/ToolIoVisibilityTest` |
| 30 | Tool arguments and results are redacted in steps, broadcasts and the audit log | `Security/SecretRedactionTest` |
...
- [ ] All 36 criteria have tests, and they pass
- [ ] `vendor/bin/pest` green
- [ ] `vendor/bin/phpstan analyse` clean at level 8
- [ ] `vendor/bin/pint --test` clean
- [ ] `docs/guides/tools.md` written, and its examples actually run
