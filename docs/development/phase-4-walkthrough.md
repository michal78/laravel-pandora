...
- [x] Both demo automations are listed.
- [x] `demo-webhook` says **"Waits for its trigger"** under Next run rather than leaving a blank cell.
      A gap reads as missing data; this is the actual state of an externally-woken automation.
- [x] Because nothing has fired on the cron yet, the page names **`schedule:run`** at the top. This
...
- [x] Press **Enable** on `demo-heartbeat`. The notice names the next run, in Europe/Copenhagen.
- [x] The Next run column shows a time about a minute out, **in Copenhagen time** — two hours ahead
...
- [x] Within two minutes, Last run changes from **Never**.
- [x] Open the automation → **History**. There is an occurrence row, `dispatched`, linking to a run.
- [x] Follow the run. Its trigger is `schedule` and it is stamped `observe_only`.
...
- [x] **Schedule** tab → Edit → set the cron expression to `every tuesday-ish` → Save. It is
      **refused with the reason**, and the stored schedule is unchanged. An unparseable expression
      ...
      never runs.
- [x] **Behaviour** tab → Edit → open the Autonomy select. The `echo` agent is `suggest`, so
      **`Act within policy` is not offered**, and the help text says why. A level that would be
      silently clamped is not a choice.
- [x] Change the autonomy budget to something small, save, and confirm the History tab records a
      `refused` occurrence with reason `autonomy_budget` once it is exhausted — and that the
      automation turns **itself** off.

## 5. The webhook panel

- [x] `demo-webhook` → **Webhook** tab. The URL and a worked signature example are both shown.
- [x] Press **Rotate secret**. The new secret appears **once**, with a warning.
- [x] Reload the page. It is **gone** — there is no second chance to read it.
- [x] Copy it before reloading and try it:
...
- [x] **Agents → Echo → Automations**. Both automations are listed.
- [x] The autonomy shown is the **effective** level, not what each automation asked for.

## 7. Authorization

- [x] In `tinker`, deny the ability for your user:
      `Gate::define('pandora.automations.manage', fn () => false);` — or remove whatever grants it.
- [x] The **New automation** button, the Enable/Disable buttons, **Run now** and the proposal queue
