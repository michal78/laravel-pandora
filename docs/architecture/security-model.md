...
| T3 | Cross-session context leak (two users, one conversation; shared channel inbox) | Session is a security boundary. Context and memory retrieval are session-scoped. Channel identity ≠ application identity without explicit linking. |
...
| T8 | Privilege escalation through delegation | Child run's effective abilities are the **intersection** of parent and child agent permissions. Delegation never widens authority. |
...
| T10 | Hostile MCP server (tool *descriptions* are an injection vector) | Remote tools untrusted until explicitly allowed per agent. Namespaced. Descriptions treated as untrusted content. Schema changes require re-approval. |
