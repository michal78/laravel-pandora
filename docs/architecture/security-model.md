...
| # | Threat | Control |
|---|---|---|
| T1 | Injected instructions in a document/webpage/tool result cause a destructive tool call | Tool calls are authorized against the *actor's* abilities, not the agent's. Tools at the risk levels in `pandora.approvals.required_for` (`high`/`critical` by default) require approval. Untrusted content is delimited with a marker it **cannot write** — `UntrustedBlock` neutralises the closing tag — labelled, and kept out of the system position. The approval card shows the arguments that will execute, plus a labelled diff of anything a policy changed. Verified end to end by `Security/InjectionToDestructiveCallTest`: a model reads a poisoned document, demands a refund, and is parked for a human. Also `UntrustedContextTest`, `ApprovalArgumentFidelityTest`, `ApprovalFloorAgreementTest`. |
